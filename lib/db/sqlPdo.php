 ...
 *
 */
class sqlPdo extends funciones
{
  ...
  */
  public function executeSimpleQuery($ps, $params = null, $query = "", $types = null, $iup = false, $cursor = false, $debug = true)
  {
    ...
    public function getLastInserted($connection)
    {
     ...
     */
    public function executeQuery($connection, $tabla, $fields = false, $condition = false, $order = false, $operation = false, $operador = false, $debug=true, $limit = false)
    {
        ...
        if ($tabla)
        {
            ...
            if ($consulta != "")
            {
                ...
                $sqlResults = $this->executeSimpleQuery ( $ps, $params, $consulta , null, false, false, $debug);
                return $sqlResults;
            }
            else
            {
...
/**
    ...
    {               
        ...
        (CVE_USUARIO, FECHA, MODULO, ACCION, DATOS, IP) 
	    	VALUES
	    	(?,SYSDATETIME(),?,?,?,?) ";        
        $ip = $_SERVER ['REMOTE_ADDR'];
        $ps = $this->setSimpleQuery ( $connection, $consulta );
        $params = array ($cveUsuario,  $modulo, $accion, $datos, $ip );
        $sqlResult = $this->executeSimpleQuery ( $ps, $params, $consulta,null,  true );
    }
}
