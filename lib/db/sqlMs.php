...
class sqlMs extends funciones
{
	...
	public function executeQueryP($connection, $consulta, $params)
    {
        
        //error_log("-->>>".$consulta);
        
    	$result = mssql_query($consulta, $connection);
    	
    	//$rs = mssql_result ($result, 0, 0);
    ...
    public function executeSP($connection, $sp, $params)
    {   
        error_log($sp);
        
        $sql_statement =  mssql_init($sp, $connection);
        
        // Vincular los nombres de campos
        mssql_bind($sql_statement, '@FechaInicio',  $params[0], SQL_DATE);
        mssql_bind($sql_statement, '@FechaFin',     $params[1], SQL_DATE);
        mssql_bind($sql_statement, '@operadora',    $params[2],  SQLVARCHAR, false, false, 20);
        mssql_bind($sql_statement, '@fondo',    $params[3],  SQLVARCHAR, false, false, 20);
